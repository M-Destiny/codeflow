...
httpServer.listen(0, async () => {
  ...
  // Now wait for user:leave when Alice disconnects - this is what the test does
  const leavePromise = new Promise((resolve, reject) => {
    const timeout = setTimeout(() => reject(new Error("Timeout waiting for user:leave")), 15000);
    client2.once("user:leave", (data: any) => {
      ...
      if (data.socketId !== client1.id) {
        console.log("WARNING: socketId mismatch but continuing test");
      }
      resolve();
    });
  });
