...
describe('SignalServer Integration', () => {
  ...
  async function joinRoom(socket: any, roomId: string, userName: string): Promise<any> {
  ...
  describe('Room Join', () => {
    ...
    it('should enforce rate limit on room:join', async () => {
      ...
      const { io: clientIO } = require('socket.io-client');
      const port = (httpServer.address() as any).port;
      const freshSocket = clientIO(`http://localhost:${port}`, { transports: ['websocket'], forceNew: true });
      ...
      for (let i = 0; i < joinCount; i++) {
        freshSocket.emit('room:join', { roomId: `room-${i}`, userName: `User${i}` });
      }
  ...
  describe('Cursor Updates', () => {
    ...
    it('should broadcast cursor updates to others in room', async () => {
      ...
      const cursorPromise = new Promise((resolve) => client2.once('cursor:update', resolve));
  ...
  describe('Operations', () => {
    ...
    it('should broadcast insert operations', async () => {
      ...
      const opPromise = new Promise((resolve) => client2.once('operation', resolve));
  ...
  describe('Chat Messages', () => {
    ...
    it('should broadcast chat messages to all in room', async () => {
      ...
      const msgPromise = new Promise((resolve) => client2.once('chat:message', resolve));
  ...
  describe('WebRTC Signaling', () => {
    ...
    it('should relay RTC offer to target peer', async () => {
      ...
      const offerPromise = new Promise((resolve) => client2.once('rtc:offer', resolve));
    ...
    it('should relay RTC answer to target peer', async () => {
      ...
      const bobSocketId = client2.id;
      const answerPromise = new Promise((resolve) => client2.once('rtc:answer', resolve));
    ...
    it('should relay ICE candidates', async () => {
      ...
      const bobSocketId = client2.id;
      const icePromise = new Promise((resolve) => client2.once('rtc:ice', resolve));
    ...
    it('should signal ICE restart to remote peer', async () => {
      ...
      const bobSocketId = client2.id;
      const restartPromise = new Promise((resolve) => client2.once('rtc:restart', resolve));
